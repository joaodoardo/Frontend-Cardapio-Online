                            ...
                            {cartItems.map(item => (
                                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #D1D5DB', borderRadius: '0.375rem' }}>
                                        <StyledButton variant="ghost" onClick={() => updateQuantity(item.id, item.quantidade - 1)} style={{ padding: '0.5rem' }}><MinusIcon style={{width: '1rem', height: '1rem'}}/></StyledButton>
                                        <span style={{ width: '2rem', textAlign: 'center', fontSize: '0.875rem' }}>{item.quantidade}</span>
                                        <StyledButton variant="ghost" onClick={() => updateQuantity(item.id, item.quantidade + 1)} style={{ padding: '0.5rem' }}><PlusIcon style={{width: '1rem', height: '1rem'}}/></StyledButton>
                                    </div>
