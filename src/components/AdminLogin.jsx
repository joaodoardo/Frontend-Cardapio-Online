...
const AdminLogin = ({ setView }) => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const { token } = await r.json(); login(token); setView('adminDashboard');
        } catch (err) { setError(err.message || 'Erro ao fazer login.'); } finally { setIsLoading(false); }
    };
        ...
        <div style={{minHeight: '100vh', backgroundColor: '#F9FAFB', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '1rem'}}>
            <div style={{width: '100%', maxWidth: '400px'}}>
                <div style={{textAlign: 'center', marginBottom: '1.5rem'}}>
                    ...
                    <h2 style={{color: 'black', marginTop: '0.5rem', fontSize: '1.875rem', fontWeight: 'bold'}}>Acesso Restrito</h2>
                ...
                <div style={{textAlign: 'center', marginTop: '1rem'}}><button onClick={() => setView('menu')} style={{background:'none', border: 'none', color: '#EA580C', textDecoration: 'underline', cursor: 'pointer'}}>Voltar ao Cardápio</button></div>
            </div>
        </div>
    );
};
